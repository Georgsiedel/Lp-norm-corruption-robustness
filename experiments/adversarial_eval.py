...
def pgd_with_early_stopping(model, inputs, labels, clean_predicted, eps, number_iterations, epsilon_iters, norm, optimization_iters):
...
def adv_distance(testloader, model, number_iterations, epsilon, eps_iter, norm, optimization_iters):
    ...
    for i, (inputs, labels) in enumerate(testloader):
        ...
        adv_inputs, adv_predicted = pgd_with_early_stopping(model, inputs, labels, predicted, epsilon, number_iterations, eps_iter, norm, optimization_iters)
...
def compute_adv_distance(testset, workers, model, adv_distance_params):
    ...
    optimization_iters = adv_distance_params["optimization_iters"]
    nb_iters = adv_distance_params["nb_iters"]
    norm = adv_distance_params["norm"]

    dst0, idx0, dst1, idx1, dst2, idx2, adv_acc = adv_distance(truncated_testloader, model, nb_iters, epsilon, eps_iter, norm, optimization_iters)

    return adv_acc*100, dst0, idx0, dst1, idx1, dst2, idx2

def compute_adv_acc(autoattack_params, testset, model, workers):
    truncated_testset, _ = torch.utils.data.random_split(testset,
                                                         [autoattack_params["setsize"], len(testset)-autoattack_params["setsize"]],
                                                         generator=torch.Generator().manual_seed(42))
    ...
    for batch_id, (inputs, targets) in enumerate(truncated_testloader):
        adv_inputs, adv_predicted = adversary.run_standard_evaluation(inputs, targets, bs=10, return_labels=True)

        for i, input in inputs:
            distance = torch.norm((input - adv_inputs[i]), p=autoattack_params['norm'])
