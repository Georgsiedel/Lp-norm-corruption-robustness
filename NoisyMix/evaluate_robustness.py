...
def evaluate(folder, dataset, save_dir):
    ...
    models = [f for f in os.listdir(folder) if f.startswith('best')] #os.path.isfile(os.path.join(folder, f))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dir", type=str, default='experiments/trained_models/NoisyMix_cifar10_models/', required=False, help='model dir')
    parser.add_argument("--dataset", type=str, default='cifar10', required=False, help='dataset')
    parser.add_argument("--batch_size", default=1000, type=int, help='batch size')
    args = parser.parse_args()

    test_batch_size = args.batch_size
    os.makedirs('results/NoisyMix_eval_results', exist_ok=True)
    evaluate(args.dir, args.dataset, 'results/NoisyMix_eval_results')
